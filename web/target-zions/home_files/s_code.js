/*
When setting props and eVars (or any s.variables) equal to one another always use dynamic variables.
When using dynamic variables always set eVar equal to the value and set prop using dynamic variable.  This allows us to use 'Real-Time' reporting to see what value is being populated on Adobe server. (eVars cannot be viewed in 'Real-Time')
Example: s.prop1 = s.eVar1 = var;
Shoule be: 	s.eVar1 = var;
			s.prop1 = "D=v1"; 
...
s.linkInternalFilters="javascript:,zionscodegreen,snl,zionsbank,ezmerchantcards,zionscheckup,zionsinsurance,zionsgoldservice,zbdirect,moneylinelending.com,vault.entervault,thevalidnetwork,draperbank,secure-loancenter.com"
s.linkLeaveQueryString=false
s.linkTrackVars="eVar6,prop6,eVar17,prop17,eVar54,prop54,eVar56,prop56"
...
s.pathExcludeList=".jsp"   // elements to exclude from the path
...
function s_doPlugins(s) {
	...
	// if error page, pre-append "404" to the referrer URL and send that in as the pagename
	if (s.pageType === "errorPage") {
		if (!s.pageName) {
            s.pageName = "404:" + s.wd.document.referrer;
        }
		s.prop11 = "D=pageName"; //Set prop11 to pageName server side (at Adobe)

		s.referrer = $.cookie("customReferrer");	//Cookie captures the referrer URL from before 404redirect.jsp (Otherwise all errorPage referrers show up as 404redirect.jsp)
		s.prop25 = $.cookie("customReferrer"); 		//prop to report referrer to troubleshoot error page referrer report

		$.cookie("customReferrer", null, { path: '/' });
        ...
        // Set s.pageName to "home" if set to index.jsp
        if (s.pageName === "index.jsp" || s.pageName === "") {
        	if(s.c_d.indexOf("mortgagewebcenter.com") > -1){
        		s.pageName = "mortgage-home";
        	}else{
        		s.pageName = "home";
        	}
        }

        // Check for index pages of a folder, set s.pageName to folder title
        var ind = s.pageName.search(":index.jsp");
        if (ind > -1) {
        	s.pageName = s.pageName.substring(0, ind);
        }

        if (s.getQueryParam('formStatus') && (s.getQueryParam('formStatus') === "sent")) {
        	s.pageName += ":confirmation";
        }
		/* External Campaign Tracking */
		//	Prevent setting of campaign variable and cookie on redirect pages
		if (!s.campaign && !s.prop15) {
			s.campaign = s.getQueryParam('zbid');
			s.campaign = s.getValOnce(s.campaign, 's_campaign', 0);
		}

		var term = s.getQueryParam('pterm');
		if (term != undefined) {
			s.eVar33 = term;
		}

		if (s.campaign) {
			s.prop10 = "campaign: " + s.campaign + " - " + s.pageName;
		}

		/* Plugin Example: linkHandler v0.5
		var ToolUrl=s.linkHandler('lf.zionsbank.com/tools|partners.leadfusion.com/tools/zionsdirect/');
		if(ToolUrl){
		s.prop4=s.eVar4=ToolUrl;
		s.events=s.apl(s.events,'event4',',',1)
		s.linkTrackEvents=s.apl(s.events,'event4',',',1)
		s.linkTrackVars="prop4,eVar4,events"
		}*/

		// Commented out on 1/24/13. Logic moved to /includes/search-page-code.js and used directly in google mini xslt
		/* Based on presence of "q" query string on search results page, set prop1 (search term), set to lowercase, copy to evar, and set deduped event */
		// s.prop1=s.getQueryParam('q');
		// if(s.prop1){
		// s.prop1=s.prop1.toLowerCase();
		// s.eVar1=s.prop1;
		// s.prop10="search: "+s.prop1+" - "+s.pageName;
		...
		//	Prevent setting of campaign variable and cookie on redirect pages
		if (!s.eVar2 && !s.prop15) {
			s.eVar2 = s.getQueryParam('zid');
			s.eVar2 = s.getValOnce(s.eVar2, 's_ev2', 0);
		}
		if (s.eVar2) {
			s.prop10 = "internal campaign: " + s.eVar2 + " - " + s.pageName;
		}

		//Counts number of campaigns user interacts with
		if ((s.getQueryParam('zid')) || (s.getQueryParam('zbid'))) {
			//increment number of campaigns counter
			s.eVar60 = "+1";
		}

		if (s.getQueryParam('code')) {
			s.eVar55 = s.getQueryParam('code');
			s.eVar55 = s.getValOnce(s.eVar55, 's_ev55', 0);
		}
		...
		/* Copy UserID from prop5 to evar18
		if(s.prop5){s.eVar18=s.prop5;} */

		/* Copy Client Status from prop6 to evar6
		if(s.prop6){s.eVar6=s.prop6;} */

		//Timeparting MST v3.4
		var tp = s.getTimeParting('n', '-7');
		tp = tp.split("|");		//Split hour/day at pipe ('|')
		s.eVar7 = tp[0];		//Set to hour (timeparting plugin)
		s.prop7 = "D=v7";		//Set to eVar7 server side (at Adobe)
		
		s.eVar8 = tp[1];		//Set to day (timeparting plugin)
		s.prop8 = "D=v8";		//Set to eVar8 server side (at Adobe)


		/* Copy Language from prop14 to evar14, AND langugae pathing prop13
		if(s.prop14){
		s.eVar14=s.prop14;
		s.prop13=s.prop14+":"+s.pageName;
		}	*/

		/*Used for campaign stacking*/
		if (s.campaign) {
			s.eVar11 = s.prop10 = s.campaign;
		}
		s.eVar12 = s.crossVisitParticipation(s.campaign, 's_cpm', '90', '5', '>', '');

		/*Copy old CID tracking codes */
		// s.eVar13=s.getQueryParam('cid');

		/*Campaign and Search Term pathing*/
		if (!s.prop10) {
			s.prop10 = s.pageName;
		}
		...
		for (var i = 0; i < cookieArray.length; i++) {
			var str = cookieArray[i].trim();
			var strArray = str.split("=");
			cookieObject[strArray[0]] = decodeURIComponent(strArray[1]);
		}
        
		//  THIS APPLIES TO ONLINE APPLICATIONS: zap, cconline, wlonline
        //  When the purchase event is set, set the transactionID, transactionID eVar(19)
        //  to purchaseID
		if (s.events && s.events.indexOf("purchase") > -1) {
            if (typeof s.purchaseID !== 'undefined' && (s.purchaseID.trim() !== "")) {
                s.purchaseID = s.channel + "-" + s.purchaseID;
                s.eVar19 = s.transactionID = s.purchaseID;
                
                //  Check if TransactionID cookie exists; update or add as required
                var xactValue = cookieObject.xact;
                // if (typeof xactValue !== 'undefined' && xactValue !== s.transactionID) {
                if (typeof xactValue === 'undefined' || xactValue !== s.transactionID ) {
                    xactValue = "xact=" + s.transactionID;
                    xactValue += ";path=/";
                    xactValue += ";domain=.zionsbank.com";
                    xactValue += ";secure";
                    document.cookie = xactValue;
                    
                    // XMLHttpRequest is supported in all major browsers including IE 7+
                    if (window.XMLHttpRequest) {
                        httpRequest = new XMLHttpRequest();                    
                        var url = document.location.protocol + "//";
                        url += document.location.hostname;
                        url += '/wlonline/images/zions/ehl_logo.png?t=' + new Date().getTime();
                ...
                s.evar19 = "PurchaseID not set";
            }
        }

        /**
         * sends out a beacon with prodview event details 
         */
		if(s.hasOwnProperty('events') && s.hasOwnProperty('products')){
    		if((typeof s.products !== 'undefined') &&  (typeof s.events !== 'undefined')){
      			if((s.events.indexOf("prodView") > -1) && (s.products.trim() !== "") ){
      				var prodviewValue = cookieObject.prodview,
      				s_prod = s.products.split(';').join('');
        			if (typeof prodviewValue === 'undefined' || prodviewValue !== s_prod ) {      
          				document.cookie = "prodview="+ s_prod +";path=/;domain=.zionsbank.com;secure";
          				var img = new Image();
						img.src = "https://www.zionsbank.com/applications/target.png?t="+ new Date().getTime();
		...
		//  Check if plid cookie exists and populate eVar34 and prop34
		var plidValue = (typeof cookieObject.plid !== 'undefined') ? cookieObject.plid : "plid not found";
		s.eVar34 = plidValue;	//set to plidValue
		s.prop34 = "D=v34";		//set to eVar34 server side (at Adobe)
		
	}

	s.eVar54 = window.location.href;	//set to URL
	s.prop54 = "D=v54";					//set to eVar54 server side (at Adobe)

	s.eVar56 = "D=pageName";			//set to pageName server side (at Adobe)
	s.prop56 = "D=v56";					//set to eVar56 server side (at Adobe)

	
	//Must be at the END of s_doPlugins.  Function is called at the beginning of s_code and end of s_doPlugins.
	s.eVar52 = s_getLoadTime();		//set to load time (getLoadTime plugin)
	s.prop52 = "D=v52";				//set to eVar52 server side (at Adobe)
...
/************************** PLUGINS SECTION *************************/
/* Same as code on SiteCatalyst code version: H.19.4 on current zions site as of 7/28/2011 - m.adoni */
...
+"'SocialAuthor');s.Integrate.SocialAuthor.tEvar='eVar3';s.Integrate"
...
s.trackingServer="metrics.zionsbank.com"
s.trackingServerSecure="smetrics.zionsbank.com"
s.visitorMigrationKey="52FEA464"
s.visitorMigrationServer="zionsbancorp.112.2o7.net"
s.visitorMigrationServerSecure="zionsbancorp.112.2o7.net"
